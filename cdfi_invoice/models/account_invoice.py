...
class AccountMove(models.Model):
    ...
    def to_json(self):
        ...
        for line in self.invoice_line_ids:
            ...
            for tax in taxes:
                ...
                if tax_id.price_include or tax_id.amount_type == 'division':
                    amount_wo_tax -= float("%.4f" % tax['amount']) #tax['amount']
                self.monto_impuesto = float("%.4f" % tax['amount']) #tax['amount']
                ...
                val = {'move_id': line.move_id.id,
                 ...
                 'tax_id': tax['id'],
                 'amount': float("%.4f" % tax['amount'])} #tax['amount']}
                key = tax['id']
            ...
            self.precio_unitario = "{:.4f}".format(float(amount_wo_tax) / float(line.quantity))
            ...
            if self.tipo_comprobante == 'E':
                invoice_lines.append({'quantity': line.quantity,
                                      ...
                                      'amount': "{:.4f}".format(line.price_subtotal),
        ...
        if self.tipo_comprobante == 'T':
            ...
            request_params['invoice'].update({'subtotal': self.truncate(self.amount_untaxed,2),'total': self.truncate(self.amount_total- self.discount,2)})
        ...
        for line in tax_grouped.values():
            ...
            tax_lines.append({
                      ...
                      'amount': float("%.4f" % line['amount']), #line['amount'],
                })
